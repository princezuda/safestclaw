"""
SafeClaw Setup Wizard - First-run interactive configuration.
...
Writes config/config.yaml and sets `safeclaw.setup_completed: true` so the
...
DEFAULT_CONFIG_TEMPLATE = """# SafeClaw Configuration
...
def is_first_run(config_path: Path) -> bool:
    """True if no config exists or `safeclaw.setup_completed` is not set."""
    ...
    try:
        ...
        return True
    return not bool((config.get("safeclaw") or {}).get("setup_completed"))
...
def _mark_completed(config_path: Path) -> None:
    """Set safeclaw.setup_completed: true in the config file."""
    config_path.parent.mkdir(parents=True, exist_ok=True)
    config: dict[str, Any] = {}
    if config_path.exists():
        try:
...
def _summary(console: Console, config_path: Path) -> None:
    ...
    lines.append("")
    lines.append("Run [bold]safeclaw[/bold] to start the interactive CLI.")
    lines.append("Run [bold]safeclaw setup[/bold] any time to re-run this wizard.")
    console.print()
    console.print(
...
async def run_wizard(
    ...
    Marks `safeclaw.setup_completed: true` at the end so subsequent launches
    don't re-prompt.
    """
    ...
    if mode == 1:
        ...
        console.print(
            ...
            "[bold]safeclaw setup[/bold] when you're ready."
        )
...
def offer_wizard_if_first_run(
    ...
    console.print(
        Panel.fit(
            "Looks like this is your first time running SafeClaw.\n"
    ...
    if Confirm.ask("Run the setup wizard now?", default=True, console=console):
        return True
    console.print(
        "[dim]No worries — run [bold]safeclaw setup[/bold] any time.[/dim]"
    )
    _ensure_default_config(config_path)
    _mark_completed(config_path)
    return False
