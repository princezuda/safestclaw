"""
SafestClaw Core Engine - The main event loop and orchestrator.
...
class SafestClaw:
    """
    Main SafestClaw engine that orchestrates all components.
    ...
    def _load_nlu(self) -> None:
        ...
        try:
            ...
            from safestclaw.core.nlu import NLUInterpreter
    ...
    def _load_languages(self) -> None:
        ...
        Reads either ``safestclaw.languages`` (list) or ``safestclaw.language``
    ...
    async def start(self) -> None:
        """Start the SafestClaw engine."""
        logger.info("Starting SafestClaw...")
        self.running = True
        ...
        if self.config.get("auto_blogs"):
            try:
                from safestclaw.core.blog_scheduler import BlogScheduler
        ...
        try:
            ...
            logger.info("SafestClaw shutting down...")
        finally:
            await self.stop()

    async def stop(self) -> None:
        """Stop the SafestClaw engine."""
        logger.info("Stopping SafestClaw...")
        self.running = False
        ...
        logger.info("SafestClaw stopped.")

    def get_help(self) -> str:
        """Return help text with available commands."""
        help_lines = [
            "SafestClaw - Your privacy-first automation assistant",
            "",
            "Available commands:",
        ]
