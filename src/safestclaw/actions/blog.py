...
from safestclaw.core.ai_writer import AIWriter, load_ai_writer_from_yaml
...
from safestclaw.core.frontpage import FrontPageManager
...
class BlogAction(BaseAction):
    ...
    def __init__(self, blog_dir: Path | None = None):
        self.blog_dir = blog_dir or Path.home() / ".safestclaw" / "blogs"
    ...
    def _ensure_initialized(self, engine: "SafestClaw") -> None:
        ...
        if config.get("publish_targets"):
            ...
            self.frontpage = FrontPageManager(
                ...
                state_dir=Path.home() / ".safestclaw" / "frontpage",
            )
    ...
    def _show_blog_menu(self, user_id: str) -> str:
        ...
        return (
            ...
            "     SafestClaw generates titles using extractive summarization.\n"
            "\n"
    ...
    async def _handle_choice(self, lower: str, user_id: str, engine: "SafestClaw") -> str | None:
    ...
    async def _handle_topic(
        self, raw_input: str, user_id: str, engine: "SafestClaw"
    ) -> str | None:
    ...
    def _looks_like_command(self, text: str) -> bool:
        """Check if text looks like a SafestClaw command rather than content."""
        command_starts = [
    ...
    async def _ai_generate(self, raw_input: str, user_id: str, engine: "SafestClaw") -> str:
    ...
    async def _setup_publish(self, raw_input: str, user_id: str, engine: "SafestClaw") -> str:
        """
    ...
    def _save_publish_target_to_config(self, target: PublishTarget, engine: "SafestClaw") -> bool:
    ...
    def _remove_publish_target(self, label: str, engine: "SafestClaw") -> str:
    ...
    async def _write_blog_news(
        ...
        if engine and hasattr(engine, "memory") and engine.memory:
            try:
                from safestclaw.core.writing_style import update_writing_profile
    ...
    async def _crawl_for_blog(
        ...
        async with httpx.AsyncClient(
            timeout=30.0,
            headers={"User-Agent": "SafestClaw/0.2 (Privacy-first crawler)"},
        ) as client:
            try:
